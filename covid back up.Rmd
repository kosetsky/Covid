...
download.file("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/03-15-2020.csv",
              "D:\\My documents\\!GoogleDrive\\Public\\covid-19\\Github\\03-15-2020.csv")


setwd("D:\\My documents\\!GoogleDrive\\Public\\covid-19\\Github\\")

files <- list.files(pattern = "*.csv", full.names = F)

crn <- do.call(bind_rows,lapply(files, function(x) read.csv(x,stringsAsFactors = F) %>% mutate(date=files[x])  )) 

for (i in files){
  if (i==files[1]) {crn<-read.csv(files[1],stringsAsFactors = F) %>% mutate(date=str_remove(files[1],".csv")) }
  crn<-read.csv(i,stringsAsFactors = F) %>% mutate(date=str_remove(i,".csv")) %>% bind_rows(crn)
}


crn <- crn %>% rename(Region=1,Country=2,last_date=3,Infected=4,Region2=10) %>%
  mutate(
    date=as.Date(date,"%m-%d-%Y"),
    #last_date=as.Date(last_date,"%m-%d-%Y"),
    Region= ifelse(is.na(Region),Region2,Region),
    Country=str_remove(Country," \\(Islamic Republic of\\)"),
  ...
  group_by(Country,date) %>% # Agreagate region within countries
  summarise(Infected=sum(Infected,na.rm = T),
            Deaths=sum(Deaths)) %>% 
  ungroup() 
```
...
crn<-read.csv("D:\\My documents\\!GoogleDrive\\Public\\covid-19\\covid_19_data.csv", stringsAsFactors = F) %>% rename(date=2,Country=4,last_date=5,Infected=6,Region=3) %>%
#  {.[.$Country == "Italy" & .$date == "2020-03-12",c("Infected","Deaths")]<-list(15113, 1016)} %>%
...
# https://www.kaggle.com/yassinehamdaoui1/coronavirus-source-data-covid19-daily-reports
...
crn <- read_excel(tf) %>% rename(date=dateRep,Country=7,Infected_new=cases,Deaths_new=deaths)  %>%
  ...
  #mutate(
        #Infected_new=ifelse(Country=="Germany"&date=="2020-03-18",2801,Infected_new),
        #Deaths_new=ifelse(Country=="Germany"&date=="2020-03-18",8,Deaths_new),
        #Infected_new=ifelse(Country=="Germany"&date=="2020-03-19",2958,Infected_new),
        #Deaths_new=ifelse(Country=="Germany"&date=="2020-03-19",11,Deaths_new),
   #     Infected_new=ifelse(Country=="Spain"&date=="2020-03-25",7871,Infected_new),
  #      Deaths_new=ifelse(Country=="Spain"&date=="2020-03-25",769,Deaths_new)
   #        ) %>% #ungroup()
  ...
  filter(!str_detect(Country,"international")) %>%
    mutate(Infected=ifelse(Country=="Italy"&date=="2020-03-14",21157,Infected),
           ...
           Infected=ifelse(Country=="Spain"&date=="2020-03-30",94417,Infected),
           Deaths=ifelse(Country=="Spain"&date=="2020-03-30",8189,Deaths),
           Infected=ifelse(Country=="Italy"&date=="2020-03-30",105792,Infected),
           Deaths=ifelse(Country=="Italy"&date=="2020-03-30",12428,Deaths),
           Deaths_new = ifelse(is.na(Deaths_new),Deaths-lag(Deaths),Deaths_new)
)
...
y<-crn3 %>% filter(Infected>inf_min) %>% 
  mutate(hjust=ifelse(Country %in% c("Germany","China"),-0.7,0.5),
         ...
         hjust=ifelse(Country %in% c("US","South Korea","Switzerland"),0.5,hjust),
         vjust=ifelse(Country %in% c("US","South Korea","Switzerland"),1.5,vjust),
         )
...
```{r include=FALSE, fig.height=6, fig.width=10}
total_death_max2<-1000
total_death_min2<-100
...
plot4new<- ggplot(x %>% filter(total>total_death_max2), aes(x=ord,y=Deaths_new)) +
